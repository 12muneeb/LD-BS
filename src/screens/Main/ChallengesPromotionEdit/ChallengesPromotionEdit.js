...
import {cities, profileData, states} from '../../../utils/dummyData';
import CustomTextInput from '../../../components/CustomTextInput';
import ProfileImage from '../../../components/ProfileImage';
import ImagePicker from '../../../components/ImagePicker';
import SocialSheetPopup from '../../../containers/Popup/socialSheetPopup/socialSheetPopup';
import {SelectList} from 'react-native-dropdown-select-list';
import {rewardList, updateChallenge} from '../../../redux/actions/appAction';
import {ASSETS_URL} from '../../../config/WebService';

const {width, height} = Dimensions.get('window');

class ChallengesPromotionEdit extends Component {
  constructor(props) {
    super(props);
    this.state = {
      Eventname: this?.props?.route?.params?.getChallengesList?.title,
      EventDescription:
        this?.props?.route?.params?.getChallengesList?.description,
      selectFormat: 0,
      date: this?.props?.route?.params?.getChallengesList?.start_date,
      date2: this?.props?.route?.params?.getChallengesList?.end_date,
      ...
      profileImage: null,
      selected: 'Eduction',
      target: this?.props?.route?.params?.getChallengesList?.target,
      rewardList: [],
    };
  }
  onSubmit = () => {
    ...
    if (Eventname == '') {
      Toast.show({
        text1: 'Event field can`t be empty',
        type: 'error',
        visibilityTime: 3000,
      });
      ...
      payload.append(
        'challenge_id',
        this?.props?.route?.params?.getChallengesList?.id,
      );
      this.props.updateChallenge(payload);
      console.log('paylaodofcooooomNow', payload);
      // Toast.show({
  ...
  render() {
    ...
    console.log('allsatattatta', JSON.stringify(this?.state));
    console.log('allsatattatta111', JSON.stringify(this?.props?.route?.params));
    ...
    const {getChallengesList} = this?.props?.route?.params;
    console.log('getChallengesListgetChallengesListInside', getChallengesList);
    console.log('ssasasasas', this?.state);

    return (
      <AppBackground
        back
        title={'Edit Challenges & Promotions'}
            ...
            <View
              ...
              <View style={{marginTop: 20}}>
                <ImagePicker
                  ...
                  <ProfileImage
                    ...
                    imageUri={
                      profileImage == null &&
                      (getChallengesList?.image == null ||
                        getChallengesList?.image == '')
                        ? appIcons.userPlaceholder
                        : getChallengesList?.image !== '' &&
                          profileImage == null
                        ? {uri: ASSETS_URL + getChallengesList?.image}
                        : profileImage?.path
                    }
                  />
                </ImagePicker>
              ...
              <View
                  ...
                  <TouchableOpacity
                    ...
                    <Text
                      ...
                      {date
                        ...
                        : moment(getChallengesList?.start_date).format(
                            'MM-DD-YYYY',
                          )}
                    </Text>
                    <Img
                  ...
                  <TouchableOpacity
                    ...
                    <Text
                      ...
                      {date2
                        ...
                        : moment(getChallengesList?.end_date).format(
                            'MM-DD-YYYY',
                          )}
                    </Text>
                    <Img
                ...
                <SelectList
                  ...
                  placeholder={
                    this?.props?.route?.params?.getChallengesList?.reward_id ==
                    1
                      ? 'Education'
                      : 'Sports'
                  }
...
const actions = {loginUser, rewardList, updateChallenge};
export default connect(mapStateToProps, actions)(ChallengesPromotionEdit);
